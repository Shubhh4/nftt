...
describe("createNFT",() =>{
  it("should create an NFT with the correct owner and tokenURI",async()=>{
    ...
    //assert that the newly created NFT's token uri is the same one sent to createNFT function
    const tokenID = receipt.events[0].args.tokenId;
    const mintedTokenURI = await nftMarket.tokenURI(tokenID);
    expect(mintedTokenURI).to.equal(tokenURI)

    //Assert that the owmer of the newly created NFT is the address that started the transaction
